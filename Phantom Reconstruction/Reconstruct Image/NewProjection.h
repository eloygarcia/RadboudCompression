...
private:
	...
	float newDet( float* a0, float* a1, float* a2);
	bool newBarycentricCoord(float* posicionPunto, float* vertex_0, float* vertex_1, float* vertex_2, float* vertex_3, float* baricentricCoordinates);
	//ImageType::Pointer compressed_image;

public:
	void getCompressedImage(std::vector<float> initial_points, 
		std::vector<float> deformed_points, 
