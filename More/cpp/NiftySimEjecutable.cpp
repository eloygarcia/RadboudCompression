...
{	
	...
	/* ISOTROPIC HIPERELASTIC NEO-HOOKEAN MATERIAL MODEL */
	// Elements Set = 1.
	...
	/* TRANSVERSE ISOTROPIC HYPERELASTIC MATERIAL MODEL */
		temp_vect.clear() ; temp_mat.clear();
		for(int i=0; i<myParameters.number_of_tissues; i++) {
			for(unsigned int j=0; j<myParameters.elementTissue.size(); j++){
				if( myParameters.elementTissue[j]== i+1 ) temp_vect.push_back( j );
			...
			if(i<2){
				for( int j_mat=0; j_mat<6; j_mat++) {
					temp_mat.push_back( myParameters.material_parameters[ j_mat ] );
				}
			} else{ 
				for( int j_mat=0; j_mat<6; j_mat++) {
					temp_mat.push_back( myParameters.material_parameters[ j_mat ] );
